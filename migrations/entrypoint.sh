...
wait_for_postgres() {
  retries=0
  until PGPASSWORD=$POSTGRES_PASSWORD psql --set=sslmode=$POSTGRES_SSLMODE -h $POSTGRES_HOST -U $POSTGRES_USERNAME -c "select 1" -d $POSTGRES_DB >& /dev/null || [ $retries -eq $MAX_RETRIES ]; do
