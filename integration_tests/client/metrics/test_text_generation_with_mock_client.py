...
@pytest.fixture
def rag_context_list() -> list[list[str]]:
    return [
        [
...
def rag_pred_answers(
    ...
    rag_context_list: list[list[str]],
) -> list[GroundTruth]:
    assert len(rag_datums) == len(rag_predictions) == len(rag_context_list)
    return [
        Prediction(
            datum=rag_datums[i],
            annotations=[
                Annotation(
                    ...
                    context_list=rag_context_list[i],
                )
            ],
        )
        for i in range(len(rag_datums))
