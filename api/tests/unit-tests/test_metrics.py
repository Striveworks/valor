...
def test__precision_and_recall_f1_from_confusion_matrix(
    ...
    (
        ...
        recall,
        f1,
    ) = _precision_and_recall_f1_from_confusion_matrix(cm, "class0")
    assert prec == 1.0
    assert recall == 1 / 3
    assert f1 == 0.5
    ...
    (
        ...
        recall,
        f1,
    ) = _precision_and_recall_f1_from_confusion_matrix(cm, "class1")
    assert prec == 0.25
    assert recall == 1.0
    assert f1 == 0.4
    ...
    (
        ...
        recall,
        f1,
    ) = _precision_and_recall_f1_from_confusion_matrix(cm, "class2")
    assert prec == 0.0
    assert recall == 0.0
    assert f1 == 0.0


def test__accuracy_from_cm(cm: schemas.ConfusionMatrix):
    assert _accuracy_from_cm(cm) == 1 / 3
