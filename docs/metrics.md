...
| Accuracy | The number of true predictions divided by the total number of predictions for a given label key. | $\dfrac{\|TP\|+\|TN\|}{\|TP\|+\|TN\|+\|FP\|+\|FN\|}$ |
| mAccuracy | The mean accuracy across all label keys. | $\dfrac{1}{\text{number of label keys}} \sum\limits_{c \in label keys} Accuracy_{c}$  |
