...
@pytest.fixture
def image():
    return {'path': 'a/b/c/d.png', 'uid': 'd', 'height': 1280, 'width': 960, 'mask_height': 640, 'mask_width': 480}
...
@pytest.fixture
def bbox1(names):
    return {'xmin': 0, 'ymin': 10, 'xmax': 20, 'ymax': 30, 'class': 0, 'confidence': 0.54}

@pytest.fixture
def bbox2(names):
    return {'xmin': 40, 'ymin': 50, 'xmax': 60, 'ymax': 70, 'class': 1, 'confidence': 0.28}

@pytest.fixture
def bbox3(names):
    return {'xmin': 80, 'ymin': 90, 'xmax': 100, 'ymax': 110, 'class': 2, 'confidence': 0.11}
...
def test_parse_image_classification(image, names):
    ...
    probs = torch.as_tensor([0.82, 0.08, 0.1])
    ...
    prediction = parse_image_classification(results)[0]
...
def test__convert_yolo_segmentation(image, yolo_mask, velour_mask):
    output = _convert_yolo_segmentation(yolo_mask, image['height']*2, image['width']*2)
    assert output == velour_mask

def test_parse_image_segmentation(image, names, bboxes, yolo_mask, velour_mask):
    img = torch.rand(image['height'], image['width'], 3)
    masks = torch.cat((yolo_mask, yolo_mask, yolo_mask), 0)

    print(masks.shape)

    results = Results(orig_img=img, path=image['path'], names=names, boxes=bboxes, masks=masks)

    predictions = parse_image_classification(results)
    ...
    for i in range(len(predictions)):
        ...
        assert predictions[i].mask == masks[i]

def test_parse_object_detection(bboxes):
    pass